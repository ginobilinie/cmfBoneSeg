...
#                 nn=nn+1
#                 volOut=sitk.GetImageFromArray(volFA)
#                 sitk.WriteImage(volOut,'Slice%d.nii.gz'%nn)
...
#                 sitk.WriteImage(volOut,'preSlice%d.nii.gz'%nn)
...
#     matOut=np.rint(matOut)
    ...
    matProb=np.transpose(matProb,(2,1,0))
...
#         volTMP=sitk.GetImageFromArray(temp)
#         sitk.WriteImage(volTMP,'temp%d.nii.gz'%myid)
